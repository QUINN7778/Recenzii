...
def performance_reviews(request):
    ...
    if request.method == 'GET':
        ...
        # Автоматическая синхронизация при создании отзыва
        perf, _ = Performance.objects.get_or_create(url=url, defaults={'title': request.data.get('title', 'Спектакль')})
        Review.objects.update_or_create(user=request.user, performance=perf, defaults={'rating': request.data.get('rating', 5), 'comment': request.data.get('comment', '')})
