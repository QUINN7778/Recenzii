...
def get_posters(request):
    try:
        ...
        response = requests.get(f"{BASE_URL}/ticket_online/", headers=headers, timeout=10)
        ...
        for element in soup.select(".cell.active"):
            ...
            if title_el:
                ...
                img = style.split("url(")[1].split(")")[0].strip("'\"") if "url(" in style else ""
                if img.startswith("/"): img = BASE_URL + img
                url = element.select_one("a")['href'] if element.select_one("a") else ""
                if url.startswith("/"): url = BASE_URL + url
                items.append({"title": title, "description": "", "date": f"{day} {month}, {time}".strip(), "imageUrl": img, "detailUrl": url})
...
def get_news(request):
    try:
        ...
        for element in soup.select(".cell"):
            text_el = element.select_one(".text") or element.select_one("a")
            title = text_el.text.strip() if text_el else ""
            date = element.select_one(".date").text.strip() if element.select_one(".date") else ""
            if title and date:
                img = element.select_one("img")['src'] if element.select_one("img") else ""
                if img.startswith("/"): img = BASE_URL + img
                items.append({"title": title, "description": "", "date": date, "imageUrl": img, "detailUrl": ""})
        return JsonResponse(items, safe=False)
    except:
        return JsonResponse([], safe=False)
...
def sync_performance(request):
    ...
    Performance.objects.update_or_create(url=url, defaults={'title': request.data.get('title', ''), 'image_url': request.data.get('image_url', ''), 'description': request.data.get('description', '')})
...
def performance_reviews(request):
    ...
    if request.method == 'GET':
        ...
        performance = Performance.objects.get(url=request.data.get('url'))
        Review.objects.update_or_create(user=request.user, performance=performance, defaults={'rating': request.data.get('rating', 5), 'comment': request.data.get('comment', '')})
